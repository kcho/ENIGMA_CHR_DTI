...
from enigmaObjPipe.diffusion.dwi import DwiPipe, DwiToolsStudy, merge_dwis
...
class EnigmaChrSubjectDicomDir(
    ...
    def __init__(self, dicom_dir):
        ...
        self.diff_raw_dwi_tmp1 = self.nifti_dir / \
                f'{self.subject_name}_DTI_2x32_36.nii.gz'
        self.diff_raw_bvec_tmp1 = self.nifti_dir / \
                f'{self.subject_name}_DTI_2x32_36.bvec'
        self.diff_raw_bval_tmp1 = self.nifti_dir / \
                f'{self.subject_name}_DTI_2x32_36.bval'
        self.diff_raw_dwi_tmp2 = self.nifti_dir / \
                f'{self.subject_name}_DTI_2x32_35.nii.gz'
        self.diff_raw_bvec_tmp2 = self.nifti_dir / \
                f'{self.subject_name}_DTI_2x32_35.bvec'
        self.diff_raw_bval_tmp2 = self.nifti_dir / \
                f'{self.subject_name}_DTI_2x32_35.bval'
    ...
    def subject_pipeline(self, force: bool = False, test: bool = False):
        ...
        self.convert_dicom_into_bids_for_penn(force)

        # 2.5 merge two dMRI data for penn data
        if not self.diff_raw_dwi.is_file():
            merge_dwis(self.diff_raw_dwi_tmp1,
                       self.diff_raw_bval_tmp1,
                       self.diff_raw_bvec_tmp1,
                       self.diff_raw_dwi_tmp2,
                       self.diff_raw_bval_tmp2,
                       self.diff_raw_bvec_tmp2,
                       self.diff_raw_dwi,
                       self.diff_raw_bval,
                       self.diff_raw_bvec)
...
class EnigmaChrStudyPenn(StudyTBSS, RunCommand, Snapshot,
    ...
    def __init__(self, root_dir: Path, site: str = None):
        ...
        if not Path(config_loc).is_file():
